...
conv_name = sys.argv[2]
conv_dir = "./" + conv_name
os.mkdir(conv_dir)

pg = 1000
query = '{"method": "read", "params": {"options": {"channel": {"name": "'+team_name+'", "members_type": "team", "topic_name": "'+conv_name+'"}, "pagination": {"num":' + str(pg) + '}}}}'
...
def outputmsgs():
    ...
    for entry in outputmsgs.json_data["result"]["messages"]:
        ...
        if ctype == "text":
            ...
            query = '{"method": "download", "params": {"options": {"channel": {"name": "'+team_name+'", "members_type": "team", "topic_name": "'+conv_name+'"}, "message_id": ' + mid + ', "output": "' + file_name + '" }}}'
...
while outputmsgs():
    query = '{"method": "read", "params": {"options": {"channel": {"name": "'+team_name+'", "members_type": "team", "topic_name": "'+conv_name+'"}, "pagination": {"next": "' + outputmsgs.next + '", "num":' + str(pg) + '}}}}'
    run_query(query)
